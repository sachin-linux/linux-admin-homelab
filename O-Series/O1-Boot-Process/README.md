...
The Boot Process is the sequence of events that takes a Linux system from power-on to a usable login prompt. It spans firmware (BIOS/UEFI), the bootloader (GRUB2), the Linux kernel, the initial RAM filesystem (initramfs), and the system (systemd) - each stage handing off control to the next.
...
**Problem:** Need to confirm which kernels and initramfs images actually exist on disk, and w00hether GRUB's menu entries match ref files.
...
**Fix:** No fix needed - observating exercise.
**Key lesson:** The linux line loads the kernel and passes it a UUID to look for, but the kernel can't act on that  alone - it's initramfs's job to actually find, driver-up, and mount that UUID, then switch_root into it. Kernel and initramfs version in a menuentry always match exactly.
...
**Investigate:** Traced the sequence stage by stage instead of assuming an early failure - confirms GRUB doesn't validate root UUID at all, it only loads files; confirmed the kernel itself boots fine and hands off to initramfs; identified initramfs as the stage that actually searches for the UUID.
**Fix:** From the (initramfs) rescue shell, run blkid to find the real current UUID, then either manually correct the root=parameter at the GRUB menu (press e to edit) to boot a live USB, chroot in, and run update-initramfs -u plus update-grub to regenerate configs with the correct UUID.
**Key lesson:** GRUB and kernel succeeding does not mean boot succeeds - initramfs is where root-identification failures actually surface, with the telltale "Gave up waiting for root device"/"ALERT!UUID=...does not exist" message.
...
- Practical recovery path for UUID-mismatch boot failure after a disk clone
